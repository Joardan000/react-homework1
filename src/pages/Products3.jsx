...
// Statik ikonkalarni import qilamiz
...
function Products() {
    ...
    const mapData = product.map((a) => (
        <div
            className="cursor-pointer hover:shadow-lg max-w-[264px] p-4 hover:max-w-[350px] flex justify-center mx-auto w-full transition-all duration-300 ease-in-out bg-pink-100 rounded-lg"
            key={a.id}>
            <div>
                <div className="relative h-[309.33px] object-cover">
                    {/* Mahsulot rasmi baseData.js da import qilingan bo'lishi kerak */}
                    <img className="rounded-lg max-w-full w-full h-[309.32px] mx-auto object-cover" src={a.img} alt={'img' + a.id}/>

                    {/* Import qilingan o'zgaruvchilar */}
