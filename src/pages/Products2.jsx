...
// Statik ikonkalarni import qilamiz (assets papkangizga yo'l to'g'riligini tekshiring)
...
function Products() {
    ...
    const mapData = product.map((a) => (
        <div
            className="cursor-pointer hover:shadow-lg max-w-[264px] p-4 hover:max-w-[350px] flex justify-center mx-auto w-full transition-all duration-300 ease-in-out bg-purple-200 rounded-lg"
            key={a.id}>
            <div>
                <div className="relative h-[309.33px] object-cover">
                    {/* Mahsulot rasmi baseData.js dagi import qilingan o'zgaruvchi orqali keladi */}
                    <img className="rounded-lg max-w-full w-full h-[309.32px] mx-auto object-cover" src={a.img} alt={'img' + a.id}/>

                    {/* Statik ikonkalar endi o'zgaruvchi ko'rinishida */}
                ...
                <div className="max-w-full px-2 pt-3 pb-2">
                    ...
                    <div
                        className="mb-[2px] bg-[#FFFF00] text-center rounded text-[11px] leading-[17px] text-[#1F1F26] max-w-[105.94px] w-full">{a.installment}</div>
